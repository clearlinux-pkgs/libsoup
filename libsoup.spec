...
Source0  : https://download.gnome.org/sources/libsoup/2.66/libsoup-2.66.0.tar.xz
...
%prep
%setup -q -n libsoup-2.66.0
pushd ..
cp -a libsoup-2.66.0 build32
popd
...
export LANG=C
export SOURCE_DATE_EPOCH=1552402233
export AR=gcc-ar
...
CFLAGS="$CFLAGS" CXXFLAGS="$CXXFLAGS" LDFLAGS="$LDFLAGS" meson --prefix /usr --buildtype=plain -Dgssapi=false  builddir
ninja -v -C builddir
pushd ../build32
...
meson --libdir=/usr/lib32 --prefix /usr --buildtype=plain -Dgssapi=false  builddir
ninja -v -C builddir
popd

%install
mkdir -p %{buildroot}/usr/share/package-licenses/libsoup
cp COPYING %{buildroot}/usr/share/package-licenses/libsoup/COPYING
pushd ../build32
...
/usr/share/package-licenses/libsoup/COPYING
